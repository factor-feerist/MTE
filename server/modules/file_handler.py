...
class FileHandler:
    ...
    def new(self, filename, username):
        ...
        with open(
                f'{self._current_directory}\\.rep\\files\\{filename}\\actual',
                'w'):
            pass
        with open(f'{self._current_directory}\\.rep\\files\\{filename}'
                  '\\actual_version', 'w') as f:
            f.write('0')
        with open(f'{self._current_directory}\\.rep\\users', 'a') as f:
            f.write(f'{filename}\\\\{username}\n')
        with open(f'{self._current_directory}\\.rep\\files\\{filename}\\log',
                  'w') as f:
            f.write(f'{filename} created by {username} at '
                    f'{datetime.now().strftime("%d/%m/%y %H:%M:%S")}\n')

    def open(self, filename, username):
        if not os.path.isdir(
                f'{self._current_directory}\\.rep\\files\\{filename}'):
            raise Exception(f'File {filename} doesn\'t exist')
        with open(f'{self._current_directory}\\.rep\\users', 'a') as f:
            f.write(f'{filename}\\\\{username}\n')
        with open(f'{self._current_directory}\\.rep\\files\\'
                  f'{filename}\\actual') as f:
            return f.read()
    ...
    def save(self, filename, username):
        ...
        with open(f'{self._current_directory}\\.rep\\files\\'
            ...
            text = f.read()
        with open(f'{self._current_directory}\\.rep\\files\\'
                  f'{filename}\\actual_version') as f:
            version = int(f.read())
        with open(f'{self._current_directory}\\.rep\\files\\'
                  f'{filename}\\actual_version', 'w') as f:
            f.write(str(version + 1))
        with open(f'{self._current_directory}\\.rep\\files\\'
                  f'{filename}\\{version}', 'w') as f:
            f.write(text)
        with open(f'{self._current_directory}\\.rep\\files\\'
                  f'{filename}\\log', 'a') as f:
            f.write(f'{filename} v.{version} saved by {username} at '
                    f'{datetime.now().strftime("%d/%m/%y %H:%M:%S")}\n')
        return version
    ...
    def watch(self, filename, version):
        if not os.path.isdir(f'{self._current_directory}\\.rep\\files\\'
                             f'{filename}'):
            raise Exception(f'File {filename} doesn\'t exist')
        if not os.path.exists(f'{self._current_directory}\\.rep\\files\\'
                              f'{filename}\\{version}'):
            raise Exception(f'File {filename} v.{version} doesn\'t exist')
        with open(f'{self._current_directory}\\.rep\\files\\'
                  f'{filename}\\{version}') as f:
            return f.read()
    ...
    def add(self, filename, username, index, to_add=''):
        ...
        with open(f'{self._current_directory}\\.rep\\files\\'
                  f'{filename}\\actual', 'w') as f:
            f.write(text)
        with open(f'{self._current_directory}\\.rep\\files\\'
                  f'{filename}\\log', 'a') as f:
            f.write(f'{to_add}\nwas added by {username} at '
                    f'{datetime.now().strftime("%d/%m/%y %H:%M:%S")}\n')
        return text
